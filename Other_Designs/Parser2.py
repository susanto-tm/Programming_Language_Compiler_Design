...
class Parser:
    ...
    def p_commands(self, p):
        ...
        commands : line_statement
                 | statement
                 | line_statement block
                 | commands line_statement
                 | commands block
        """
        if len(p) > 2:
            debug("COMMANDS", p[1], p[2])
            p[0] = p[1] + [p[2]]
        else:
            p[0] = p[1]
    ...
    def p_statement_condition(self, p):
        """
        conditional_block : IF LPAREN condition_list RPAREN LBRACE line_statement RBRACE %prec IFX
                          | conditional_block ELSE LBRACE line_statement RBRACE %prec IFX
        """
        if len(p) > 6:
            debug("IF CONDITION", [str(x) for x in p[1:]])
